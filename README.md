...
There are **158 tools**. They're grouped by domain below.
...
| `ebay_create_numbered_listings` | Build **N numbered Shopify products from one image** and list them all on eBay in a single call (e.g. "Back Issue Blow Out #1..96"). The image is uploaded to Shopify Files **once** (any public URL; Google Drive share links auto-normalized) and its CDN URL reused for every product + eBay — no per-item fetches. Idempotent (existing SKU/published offer reused). Returns item numbers in order (`itemNumbersText`, one per line). A live run of >20 runs as a **background job** and returns a `jobId`; `dryRun` (default) and ≤20 return synchronously. |
| `ebay_job_status` | Check a background job started by a bulk tool (`jobId`) — status, progress, and the full result payload once done. |
...
| `ebay_order_sync_now` | Import recent eBay sales into Shopify draft orders on demand — the same job the scheduler runs twice daily (default 8 AM & 2 PM CDT). One draft per buyer **per shipment window** — a buyer who already has an open draft from an earlier day gets a NEW draft, never a cross-day append (the cron uses `separateFromExistingDrafts:true`; that's also the tool default now). `minOrdersToMerge:1` so single-order stragglers are caught. Only pulls **unshipped** eBay orders (FULFILLED ones are never merged). **Idempotent** — skips any order already in Shopify: a merge draft/order (id in note), an individually auto-synced order (eBay id = order name), or a hand-imported order (id in note prose), so overlapping runs never double-import. `skippedDetail` (in dryRun) reports each skip and why. On by default (`EBAY_ORDER_SYNC_ENABLED`); `dryRun` previews. |

...
| `ebay_merge_sales_to_draft_orders` | Group a local day's eBay sales by buyer and create ONE Shopify draft order per buyer with ≥ `minOrdersToMerge` orders — SKUs resolved to variants, no-SKU listings kept as custom lines, shipping address from the eBay ship-to. Read-only against eBay; `dryRun` previews. A note marker makes re-runs safe (already-merged buyers are skipped). |

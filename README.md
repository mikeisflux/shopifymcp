...
| `ebay_order_sync_now` | Import recent eBay sales into Shopify draft orders on demand — the same job the scheduler runs twice daily (default 8 AM & 2 PM CDT). One draft per buyer, `minOrdersToMerge:1` so single-order stragglers are caught, folding into a buyer's existing open draft. Only pulls **unshipped** eBay orders (FULFILLED ones are never merged). **Idempotent** — skips any order already in Shopify: a merge draft/order (id in note), an individually auto-synced order (eBay id = order name), or a hand-imported order (id in note prose), so overlapping runs never double-import. `skippedDetail` (in dryRun) reports each skip and why. On by default (`EBAY_ORDER_SYNC_ENABLED`); `dryRun` previews. |

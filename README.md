...
| `ebay_order_sync_now` | Import recent eBay sales into Shopify draft orders on demand — the same job the scheduler runs twice daily (default 8 AM & 2 PM CDT). One draft per buyer, `minOrdersToMerge:1` so single-order stragglers are caught, folding into a buyer's existing open draft. **Idempotent** — any eBay order already in a merge draft *or a completed merge order* is skipped (order-level), so overlapping runs (and a second twice-daily pass) never double-import. On by default (`EBAY_ORDER_SYNC_ENABLED`); `dryRun` previews. |

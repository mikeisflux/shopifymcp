...
| `shopify_normalize_book_variants` | **Bulk/domain/destructive:** normalize book products to the 5-cover set, MERGING standalone sibling listings (Foil/Metal/GITD/RM) into the base product, carrying inventory across, then deleting the emptied sibling. Optional `excludeAbovePrice` (skip premium covers) and per-cover `weights` (oz, applied to existing variants too). `dryRun` defaults on and reports deletes/inventory-moves/reprices/reweights separately; on execute a sibling is deleted only after its stock is verified on the new variant. |
